...
class StreamMessageWidgetState extends State<StreamMessageWidget>
  ...
  List<StreamChatContextMenuItem> messageActionItems({required Message message, double? iconSize}) {
    ...
    return [
      ...
      if (shouldShowReplyAction) ...[
        StreamChatContextMenuItem(
          leading: StreamSvgIcon(icon: StreamSvgIcons.reply, size: iconSize),
      ...
      if (widget.showMarkUnreadMessage)
        StreamChatContextMenuItem(
          leading: StreamSvgIcon(icon: StreamSvgIcons.messageUnread, size: iconSize),
      ...
      if (shouldShowThreadReplyAction)
        StreamChatContextMenuItem(
          leading: StreamSvgIcon(icon: StreamSvgIcons.threadReply, size: iconSize),
          title: Text(context.translations.threadReplyLabel),
          onClick: () {
            if (!isDesktopDeviceOrWeb) {
              Navigator.of(context, rootNavigator: true).pop();
      ...
      if (shouldShowCopyAction)
        StreamChatContextMenuItem(
          leading: StreamSvgIcon(icon: StreamSvgIcons.copy, size: iconSize),
      ...
      if (shouldShowResendAction)
        StreamChatContextMenuItem(
          leading: StreamSvgIcon(icon: StreamSvgIcons.sendMessage, size: iconSize),
          title: Text(
  ...
  void showMessageReactionsModal(
    BuildContext context,
    Message message,
  ) {
    final channel = StreamChannel.of(context).channel;
